...
mytextclock = awful.widget.textclock("  %Y.%m.%d (%a) %H:%M  ", 60)
...
--    awful.key({ modkey,           }, "w", function () mymainmenu:show({keygrabber=true}) end),
    ...
    awful.key({ modkey,           }, "s",
	    ...
	    for i, tag in ipairs(tags[current_screen]) do
	    ...
	    for i, tag in ipairs(tags[target_screen]) do
	        if tag.selected then
		    target_tag = i
		    break
		end
            end
	    local cclients = tags[current_screen][current_tag]:clients()
	    local tclients = tags[target_screen][target_tag]:clients()
	    for i, c in ipairs(cclients) do
	        awful.client.movetoscreen(c, target_screen)
            end

	    for i, c in ipairs(tclients) do
	        awful.client.movetoscreen(c, current_screen)
	    end
